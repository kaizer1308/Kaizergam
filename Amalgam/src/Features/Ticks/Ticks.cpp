...
void CTicks::Draw(CTFPlayer* pLocal)
{
	...
	float flTargetRatio = m_iMaxShift > 0 ? float(iTicks) / m_iMaxShift : 0.0f;
	...
	if (iTicks >= m_iMaxShift) { // Use m_iMaxShift for comparison
		if (m_iWait) {
			statusText = "Wait";
	...
	statusText = std::format("{} ({}/{})", statusText, iTicks, m_iMaxShift);
